/**
 * \file drive.cpp
 *
 * \brief Contains drive logic in user control and initialization
 * 
 * Updated - 11/30/2024
 * Last Successful Test - 11/30/2024
 */

#include "main.h"

//WIP
/**
 * @brief Initialize Drive
 * 
 */
void Nova::Drive::initialize() {
    this -> resetMotorEncoders();
    this -> setBrakeMode(pros::E_MOTOR_BRAKE_COAST);

    Nova::imu1.reset();
    Nova::imu2.reset();
}

/**
 * @brief Reset Motor Encoders
 * 
 */
void Nova::Drive::resetMotorEncoders() {
    Nova::drive.tare_position_all();
}

/**
 * @brief Change brake mode of drive
 * 
 * @param brakeMode 
 */
void Nova::Drive::setBrakeMode(pros::motor_brake_mode_e_t brakeMode) {
    Nova::drive.set_brake_mode_all(brakeMode);
}

/**
 * @brief Get average encoder value of the drive
 * 
 * @return float 
 */
float Nova::Drive::getAvgEncoderValue() {
    return (
        Nova::frontLeft.get_position() + 
        Nova::middleLeft.get_position() + 
        Nova::backLeft.get_position() + 
        Nova::frontRight.get_position() + 
        Nova::middleRight.get_position() + 
        Nova::backRight.get_position()
    ) / 6;
}

/**
 * @brief Get average velocity value of the drive
 * 
 * @return float 
 */
float Nova::Drive::getAvgVelocity() {
    return (
        Nova::frontLeft.get_actual_velocity() + 
        Nova::middleLeft.get_actual_velocity() + 
        Nova::backLeft.get_actual_velocity() + 
        Nova::frontRight.get_actual_velocity() + 
        Nova::middleRight.get_actual_velocity() + 
        Nova::backRight.get_actual_velocity()
    ) / 6;
}

/**
 * @brief Get average IMU rotation; orientiation of the drive
 * 
 * @return double 
 */
double Nova::Drive::getIMURotation() {
    return (Nova::imu1.get_rotation() + Nova::imu2.get_rotation()) / 2;
}

/**
 * @brief Logarthmic Drive
 * 
 * @param input 
 * @return double 
 */
double Nova::Drive::smooth(double input) {
    const int CURVECONSTANT = 10;

    return  (std::pow(2.718, -(CURVECONSTANT / 10)) + std::pow(2.718, (std::abs(input) - 127) / 10) * (1 - std::pow(2.718, -(CURVECONSTANT / 10)))) * input;
}

/**
 * @brief Run the drive
 * 
 */
void Nova::Drive::run() {
    int power = smooth(Nova::ctr.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_Y));
    int turn = smooth(Nova::ctr.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_X));

    leftDrive.move_voltage((power + turn) * (12000.0 / 127));
    rightDrive.move_voltage((power - turn) * (12000.0 / 127));
}